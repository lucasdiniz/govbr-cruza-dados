...
# JS load order (driven by web/static/js/_load_order.txt, generated by split_js.py).
# Each line is a path relative to /static/js/. Loaded in order via individual
# <script> tags so that script-global functions stay accessible across files.
_js_load_order = (_dir / "static" / "js" / "_load_order.txt")
if _js_load_order.exists():
    JS_FILES = [
        line.strip()
        for line in _js_load_order.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
else:
    JS_FILES = []
templates.env.globals["JS_FILES"] = JS_FILES
templates.env.globals["ASSET_VERSION"] = "55"
