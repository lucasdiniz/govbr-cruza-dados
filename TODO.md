...
- [x] **Colisoes CPF/CNPJ no cnpj_basico (completo)** — CPFs armazenados como 14 digitos compartilham cnpj_basico com CNPJs reais. Todas as correcoes aplicadas:
  ...
  - [x] **(PR feat/etl-cnpj-basico-fix)** Fix de raiz no ETL + propagacao via REFRESH MATERIALIZED VIEW CONCURRENTLY (zero downtime, sem mv_swap por MV). `etl/15_normalizar.py` Fase 5/7 com EXISTS guard preventivo + Fase 9 nova com 2 UPDATEs: (1) anula cnpj_basico contaminado retroativo; (2) extrai cpf_digitos com DV check matematico (is_valid_cnpj() + is_valid_cpf() em PL/pgSQL modulo 11). Garante que MEIs/CNPJs reais nao-sincronizados em RFB (DV CNPJ valido) NAO virem CPFs sinteticos — ficam aguardando RFB sync, ETL idempotente popula retroativamente. sql/15a (migration standalone), sql/15b (UNIQUE INDEX mv_q67_dated_pb com NULLS NOT DISTINCT + dupes guard), sql/15c (rebuild atomico _tmp_fornecedor_gov + _tmp_conflito pra mv_servidor_pb_risco). Workflow inputs `run_normalize_fix`, `refresh_mvs`, `rebuild_tmp_for_servidor`. ADR-0007. Cobre 6 MVs alem de `mv_empresa_pb` (ja corrigido na PR #153).
