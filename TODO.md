...
- [x] **Colisoes CPF/CNPJ no cnpj_basico (completo)** — CPFs armazenados como 14 digitos compartilham cnpj_basico com CNPJs reais. Todas as correcoes aplicadas:
  ...
  - [x] **(PR feat/etl-cnpj-basico-fix)** Fix de raiz no ETL: `etl/15_normalizar.py` Fase 5/7 com EXISTS guard nos UPDATE de `cnpj_basico` + Fase 9 nova de cleanup retroativo. `sql/15a_fix_cnpj_basico_contamination.sql` migration standalone. `sql/15b_add_unique_index_mv_q67.sql` adiciona UNIQUE INDEX em `mv_q67_dated_pb` (pre-flight pra REFRESH CONCURRENTLY). Workflow inputs `run_normalize_fix` + `refresh_mvs`. ADR-0007. Propagacao via REFRESH MATERIALIZED VIEW CONCURRENTLY (zero downtime) em vez de mv_swap MV-por-MV. Cobre `mv_empresa_governo`, `mv_q67_dated_pb`, `mv_municipio_pb_kpi_score`, `mv_municipio_pb_mapa`, `mv_municipio_pb_risco`, `mv_pessoa_pb` (alem de `mv_empresa_pb` ja corrigido na PR #153).
